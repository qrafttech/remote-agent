...
check "the prompt says container, sidecars up, the rest through one subagent, commit as you go, push with GH_TOKEN or the workflow will, none of it for a subagent, then the user's prompt" 'head -1 "$T/claude.prompt" | grep -q "^This is a run of app in its own container" && grep -q "docker-compose.yml declares are already up" "$T/claude.prompt" && grep -q "bring the rest of the stack up, from the repository.s own instructions, inside this checkout, through one subagent rather than in your own turns:.*detached so they outlive it.*hands back only the addresses, how to sign in and where the logs are" "$T/claude.prompt" && grep -q "GH_TOKEN in your environment.*push and open pull requests yourself, as drafts.*gh stack.*left unpushed.*the workflow pushes.*branch you leave checked out" "$T/claude.prompt" && grep -q "^These lines address the session alone: a subagent that reads them does the one task it was handed and neither runs a skill, commits nor pushes.$" "$T/claude.prompt" && tail -1 "$T/claude.prompt" | grep -qx "Implement plan.md"'
...
check "without docker-compose.yml the prompt says to bring it all up; a registered MCP is not added twice" 'grep -q "^Bring the whole stack up, from the repository.s own instructions, inside this checkout, through one subagent" "$T/claude.prompt" && ! grep -q "mcp add" "$STUBLOG"'
...
verdict session
check "action.yml: the session container is capped below the host's memory, so running out kills inside it and never the job or the runner" 'yml "s=YAML.load_file(ARGV[0])[%q(runs)][%q(steps)][0][%q(run)]; exit(s.include?(%q(MemTotal)) && s.include?(%q(--memory)) ? 0 : 1)"'
