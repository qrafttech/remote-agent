...
echo '{"b":2,"projects":{"/elsewhere":{"hasTrustDialogAccepted":true}}}' > "$T/home/.claude.json"
...
check "the checkout pre-trusted and its .mcp.json servers enabled, the rest of .claude.json kept" 'node -e "const s=require(\"$HOME/.claude.json\"),p=s.projects[\"$T/repo\"];process.exit(p.hasTrustDialogAccepted===true&&p.enableAllProjectMcpServers===true&&s.b===2&&s.projects[\"/elsewhere\"].hasTrustDialogAccepted===true?0:1)"'
