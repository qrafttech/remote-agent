...
One file, `.github/workflows/cloud.yml`, copied from `workflow.yml` here, plus the repository secrets it names. Its steps are a checkout and `uses: qrafttech/remote-agent@main`, the composite action `action.yml` of this repository, which holds the shell. Nothing else in the project exists for the runner's sake: the job brings `docker-compose.yml` up as is, and the session brings the rest of the stack up from the project's own README and `CLAUDE.md`.
...
Everything else stays: `permissions`, `concurrency`, `timeout-minutes`, the checkout without credentials, the `uses:` of this repository's action with the prompt, `fresh` and `token: ${{ secrets.CLOUD_TOKEN || github.token }}`. The action's two steps are the session step (Compose up from `docker-compose.yml` with its ports reset, then `docker run` of the agent image on that network) and the last step; they see the job's `env:`, which is why this is a composite action and not a reusable workflow, whose steps would not. The sidecars are never retyped: the step reads `docker-compose.yml` as is, and a project without one skips Compose and gets a prompt that says to bring the whole stack up.
...
- One script, `session`, one job: the step that launches and holds the session. What happens before it (sidecars) and after it (commit, push, pull request) is the action's, in `action.yml`, in plain `run:` steps; the project's `cloud.yml`, templated by `workflow.yml`, carries only its `env:` block, the checkout and the `uses:` of the action, no `run:`. What happens on the client before the dispatch (refuse a dirty tree, the host's login, the sessions it holds, push, dispatch, watch, report) is the `/cloud` skill's, `skills/cloud/SKILL.md`, written as literal shell with its two judgment points marked. A behaviour goes in the one of the three that owns it, not in a new file.
- `test.sh` is the specification of `session`. A behaviour change adds or changes a `check` line first; the last two check the shape of `action.yml` and `workflow.yml` with Ruby's YAML; the `claude` stub grows only what a check needs. `bash test.sh` must pass before a push; `bash test.sh image` after any change to the `Dockerfile`; `image.yml` runs both before it pushes the image, on a push to `main` and whenever Claude Code has a release the registry lacks; the image carries that release, every other tool at its pin.
- `session` carries a short header (what it is) and a comment above each helper saying what it is for. `action.yml` carries a short header (what it is). `workflow.yml` carries one comment above the block a project edits. No other comments: what a line does is said by the line, and what a reader would get wrong goes in the README.
...
```
job                                          steps on the host, action.yml's after the checkout
...
6. **On any exit**, SIGTERM included: stop the session if it is still going (`claude stop`). Never `claude rm`: the session stays listed on the host, for the next run of the branch to resume and for the app to show. Committing what it left is the action's last step.
...
The second step of `action.yml`, `if: always()`: the step runs after a session that ended, was stopped in the app, cancelled or timed out. The runner's source says a cancel re-evaluates the running composite step's condition, signals it and then runs the `always()` one, as it does for job steps; the probe of that on the real host is not made yet. Both steps run under `shell: bash`, which is `bash --noprofile --norc -e -o pipefail`: a job `run:` step ran without `pipefail`, so a failing command on the left of a pipe now fails the step where it used to yield an empty string.
...
4. If `HEAD` is what `origin` already has for `<branch>`: print `nothing left unpushed on <branch>` and stop — no push, no pull request.
...
What does have Docker is the runner's user, and so the steps of `cloud.yml` on the dispatched branch: a session can edit that file, and a `uses:` it rewrites or a `run:` it adds executes on the host, so read the diff of `.github/` in the pull request before dispatching the branch again. Pushing such an edit takes the token's *Workflows* permission (README §3.5); without it the push is refused and the commit stays in the runner's checkout. The shell it calls is not in the project: it is `action.yml` on this repository's `main`, protected, out of the token's reach. `@main` takes every push to `main` on the next run; a project that wants to choose when pins a tag or a commit instead.
...
- **A composite action, not a reusable workflow**: a composite action's steps see the calling job's `env:`, so `toJSON(env)` still enumerates the project's configuration; a reusable workflow's do not. Every project runs the same shell, fixed in one place.
...
Left: a `blocked` session answered from the app — the sessions were watched there, none was asked a question. A resumed session on the real host: `claude --bg --resume` is in the CLI's help, and the stub proves the call, but the probe of a second run on a branch that has a session is not made yet. The composite action on the real host: that `toJSON(env)` in its step holds the job's `env:` and nothing else, and that its `always()` step runs after a `gh run cancel` as the workflow's did.
