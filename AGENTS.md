...
- `test.sh` is the specification of `session`. A behaviour change adds or changes a `check` line first; the last two check the shape of `action.yml` and `workflow.yml` with Ruby's YAML; the `claude` stub grows only what a check needs. `bash test.sh` must pass before a push; `bash test.sh image` after any change to the `Dockerfile`; `image.yml` runs both before it pushes the image.
...
Left: a `blocked` session answered from the app — the sessions were watched there, none was asked a question. A resumed session on the real host: `claude --bg --resume` is in the pinned CLI's help, and the stub proves the call, but the probe of a second run on a branch that has a session is not made yet. The composite action on the real host: that `toJSON(env)` in its step holds the job's `env:` and nothing else, and that its `always()` step runs after a `gh run cancel` as the workflow's did.
