...
Everything else stays: the `container:` block (image, the two mounts, the seed), `permissions`, `concurrency`, `timeout-minutes`, the checkout without credentials, the `session` step, the last step. A project without `docker-compose.yml` deletes `services:` and gets a prompt that says to bring the whole stack up.
