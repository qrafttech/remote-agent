...
| `base.yml` | the `agent` service every project extends: the mounts, the stop grace period, the CPU and memory caps |
...
A project brings one file, `cloud.yml` at its root: the `agent` service plus its sidecars, and the ports its local stack publishes reset (§3.4).
...
├─ cloud, base.yml, Dockerfile           rsynced from this repository by every cloud verb
├─ home/                                 the agent's $HOME, shared by every run: login, settings, skills, pnpm store
└─ projects/<project>/
...
cloud build                                                            # Mac: ships the tooling, builds image `agent` on the host
```
...
2. **The Mac's Claude setup**, from the Mac, rerun after changing it:
   ```bash
   rsync -a ~/.claude/settings.json ~/.claude/rules ~/.claude/skills vps:/opt/agent/home/.claude/
   rsync -a --exclude repos --exclude cache ~/.claude/plugins/ vps:/opt/agent/home/.claude/plugins/
   ```
   Every session start turns the sandbox off in that `settings.json` and registers the chrome-devtools MCP with the image's Chromium, whatever the copy says.
3. **The project's configuration**, once per project, on the host: `/opt/agent/projects/<project>/env`, `chmod 600`, one `KEY=value` per line, the app's own variables with each sidecar at its service name. Only dev credentials go in it: nothing that reaches a real bucket, a real database or a real mailbox. No `.env` file exists in the tree, so none can be committed. `cloud run` refuses until the file exists.
4. **The project's overlay**, once per project, in its repository at `cloud.yml`: the `agent` service extending `${AGENT_ROOT}/base.yml` with `env_file: env` and a `depends_on` per sidecar, and for each service of the repository's `docker-compose.yml` that publishes a port, `ports: !reset []`. The sidecars themselves are never redeclared: `stack.yml` is the repository's own file, and only that name is read (`compose.yaml` is not). A repository without one gets no sidecars and a prompt that says so.
   ```yaml
   services:
     agent:
       extends:
...
- **The Mac's Claude setup**: rerun step 3.2.
