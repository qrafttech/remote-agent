...
The session's prompt gets four preamble lines: it runs in its own container, the declared sidecars are already up (or it must bring the whole stack up), commit as you go and push and open pull requests when the task calls for it — `GH_TOKEN` is in its environment and `gh stack` on its path — or leave it to the workflow, and none of that for a subagent. Exact wording in `session`. The last line is there because the Workflow tool relays the session's prompt to every subagent it spawns, as the user's voice that wins over the task text: without it a capture worker told to "run the implement-loop skill" and "push" did exactly that, on its own, before any gate had run.
